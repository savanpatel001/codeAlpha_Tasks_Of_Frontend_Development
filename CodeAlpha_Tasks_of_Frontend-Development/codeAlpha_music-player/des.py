"""
Minimal DES-ECB for JioSaavn URL decryption.
Pure Python, no external deps. Based on the DES spec.
"""
import struct
...
def _feistel(r, key):
    ...
    x = _xor(er, key)
    s = _sbox_sub(x)
    return _permute(s, _P)

def _des_process_block(block_bytes, keys):
    bits = _bytes_to_bits(block_bytes)
    ip = _permute(bits, _IP)
    l, r = ip[:32], ip[32:]
    for key in keys:
        new_r = _xor(l, _feistel(r, key))
        l = r
        r = new_r
    combined = _permute(r + l, _FP)
    return _bits_to_bytes(combined)

def des_ecb_decrypt(data, key):
    key_bits = _bytes_to_bits(key)
    keys = _generate_keys(key_bits)[::-1]
    result = b''
    for i in range(0, len(data), 8):
        result += _des_process_block(data[i:i+8], keys)
    # PKCS5 unpad
    pad = result[-1]
    if 1 <= pad <= 8:
        result = result[:-pad]
